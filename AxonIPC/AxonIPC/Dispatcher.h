...
namespace AxonIPC
{
  ...
  public:
   ...
   ~Dispatcher() = default;
    ...
    void Dispatch(int type, const std::string_view& publisher, const std::string_view& payload);

    void DeRegisterSubscriber(int type);

    template <typename Obj>
    void RegisterSubscriber(int type, void(Obj::* method)(int, const std::string_view&, const std::string_view&), Obj* obj)
    {
  ...
  protected:
    std::mutex m_mutex{};
    std::map<int, std::function<void(int, const std::string_view&, const std::string_view&)> > m_subscribers{};
  };
}
