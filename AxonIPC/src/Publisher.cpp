...
namespace AxonIPC
{
  Publisher::Publisher(Context& context, const Path& subscriberPath)
    : m_socket(context)
  ...
  void Publisher::Publish(const int type, const std::string_view& payload, const Path& publisherPath)
  {
    ...
    if (!m_socket.ContextHandle()->SendTo(m_socket.SocketHandle(), m_buf.data(), writer.Size(), publisherPath))
